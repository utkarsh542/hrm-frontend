...
export default function SmartHirePage() {
  ...
  const saveCandidate = async (applyToJob: boolean) => {
    ...
    try {
      ...
      formData.append('file', file);
      const token = typeof window !== 'undefined' ? localStorage.getItem('hrms_token') : null;
      const url = applyToJob && jobId ? `${API}/resume/parse-and-create?job_id=${jobId}` : `${API}/resume/parse-and-create`;
  ...
  return (
    <div className="animate-fade-in" style={{ padding: '24px 32px' }}>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: 16, padding: 24, border: '1px solid var(--border)', marginBottom: 24 }}>
          ...
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: 16, marginTop: 8 }}>
            <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end' }}>
              ...
              <button className="btn btn-primary" onClick={() => saveCandidate(true)} disabled={saving || !jobId} style={{ marginBottom: 0, display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      {/* Success */}
      {saved && (
        <div style={{ background: 'rgba(16,185,129,0.1)', borderRadius: 16, padding: 24, border: '1px solid rgba(16,185,129,0.3)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <PartyPopper size={22} style={{ color: '#10b981' }} />
            <span style={{ fontWeight: 700, fontSize: 18, color: '#10b981' }}>Candidate {saved.is_existing ? 'Updated' : 'Created'} Successfully!</span>
          </div>
          <p style={{ margin: 0, color: 'var(--text-secondary)' }}>
            <strong>{saved.candidate_name}</strong> (ID: {saved.candidate_id})
            {saved.application_id && ` — Applied to job (Application #${saved.application_id})`}
          </p>
        </div>
      )}
